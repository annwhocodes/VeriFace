...
def vids(
    ...
    if os.path.isfile(root_dir):  # If it's a single file
        video_files = [root_dir]
    elif os.path.isdir(root_dir):  # If it's a directory
        video_files = [os.path.join(root_dir, filename) for filename in os.listdir(root_dir)]
    else:
        raise FileNotFoundError(f"Path '{root_dir}' is neither a file nor a directory.")
    
    for curr_vid in video_files:
        try:
            if is_video(curr_vid):  # Check if the file is a valid video
...
def dfdc(
    ed_weight,
    vae_weight,
    root_dir="deepfake-detection-challenge/train_sample_videos",
    dataset=None,
    num_frames=15,
    net=None,
    fp16=False,
